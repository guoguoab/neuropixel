...
if args.use_merge_region_data:
    ...
    if len(glut_points) > 0:
        merge_legend_entries.append(["Glut (red)", "red"])
    if len(gaba_points) > 0:
        merge_legend_entries.append(["GABA (blue)", "blue"])
...
if args.region and args.region.upper() != "ALL":
    ...
    if len(unit_points) > 0:
        ...
        plotter.add_legend(
            merge_legend_entries + [["Neuropixels units", "green"]],
            loc="center right",
            bcolor=(1, 1, 1),
    ...
    if legend_entries:
        plotter.add_legend(
            merge_legend_entries + legend_entries,
            bcolor=(1, 1, 1),
            face="circle",
            size=(0.25, 0.8),
            loc="center right",
